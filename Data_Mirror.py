...
try:
    #   Excel File Name
    df1 = "Data Mirror October 2024.xlsx"
    #   Excel Sheet Name
    D1 = "Interest"
    D2 = "Profit"
    ...
    sql_query = """INSERT INTO [log_apps_error] (
                    [logerror_desc],
                    [iduser],
                    [dateerror],
                    [page],
                ...
                """
    cursor.execute(sql_query,(str(e),"Upload Excel"))
    conn.commit() 
...
try:
    ...
    sql_query = """INSERT INTO [log_apps_error] (
                ...
                """
    cursor.execute(sql_query,(str(e),"Process Excel"))
    conn.commit()
...
try:
    ...
    create_table_query = "CREATE TABLE PROFIT_N_OTHER_PAYMENT (" + ', '.join(column_types) + ")"
    # Execute the query
    cursor.execute(create_table_query)

    for row in combine2.iterrows():
        sql = "INSERT INTO PROFIT_N_OTHER_PAYMENT({}) VALUES ({})".format(','.join(combine2.columns), ','.join(['?']*len(combine2.columns)))
        cursor.execute(sql, tuple(row[1]))
    conn.commit()

    cursor.execute("""MERGE INTO col_facilities_application_master AS target USING PROFIT_N_OTHER_PAYMENT AS source
    ...
    cursor.execute("drop table PROFIT_N_OTHER_PAYMENT")
    conn.commit() 
    ...
    sql_query = """INSERT INTO [log_apps_error] (
                ...
                """
    cursor.execute(sql_query,(str(e),"Update Database"))
    conn.commit()
