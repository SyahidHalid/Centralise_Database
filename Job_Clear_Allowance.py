...
try:
    ...
    SET [jobStartDate] = getdate(), [jobStatus]= 'PY001', [PythonFileName]='Job_Clear_Allowance.py', [jobCompleted] = NULL
    WHERE [jobName] = 'Job Clear Allowance';
                """
...
try:
    ...
    sql_query2 = """INSERT INTO [log_apps_error] (
                ...
                """
    cursor.execute(sql_query2,(str(e)+" ["+str(documentName)+"]","Allowance",uploadedByEmail))
    conn.commit()
    sql_error = """UPDATE [jobPython]
    SET [jobCompleted] = NULL, [jobStatus]= 'PY003', [jobErrDetail]= 'Upload Job Clear Allowance'
    WHERE [jobName] = 'Job Clear Allowance';
                """
    cursor.execute(sql_error)
    conn.commit()
    print(f"Upload Job Clear Allowance Error: {e}")
    sys.exit(f"Upload Job Clear Allowance Error: {str(e)}")
...
try:
    #table        
    columns = ['aftd_id','result_file_name','processed_status_id','status_id']
    data = [(documentId,"Result_Allowance_"+str(convert_time)[:19]+".xlsx",'PY005','PY002')]
    ...
    for row in LDB_prev1.iterrows():
        sql = "INSERT INTO A_ALLOWANCE({}) VALUES ({})".format(','.join(LDB_prev1.columns), ','.join(['?']*len(LDB_prev1.columns)))
    ...
    sql_query5 = """INSERT INTO [log_apps_error] (
                ...
                """
    cursor.execute(sql_query5,(str(e)+" ["+str(documentName)+"]","Update Database Job Clear Allowance",uploadedByEmail))
    conn.commit()
    sql_error = """UPDATE [jobPython]
    SET [jobCompleted] = NULL, [jobStatus]= 'PY003', [jobErrDetail]= 'Update Job Clear Database Allowance'
    WHERE [jobName] = 'Job Clear Allowance';
                """
    cursor.execute(sql_error)
    conn.commit()
    print(f"Update Database Job Clear Allowance Error: {e}")
    sys.exit(f"Update Database Job Clear Allowance Error: {str(e)}")
