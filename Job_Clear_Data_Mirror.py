# python Job_Clear_Data_Mirror.py 9 "DataMirrorMay2025.xlsx.xlsx.xlsx" "Job Clear Data Mirror" "Pending Processing" "0" "syahidhalid@exim.com.my" "2025-05-31"
...
try:
    ...
    SET [jobStartDate] = getdate(), [jobStatus]= 'PY001', [PythonFileName]='Job_Clear_Data_Mirror.py', [jobCompleted] = NULL
    WHERE [jobName] = 'Job Clear Data Mirror';
                """
...
try:
    ...
    LDB_prev1['acc_others_charges_payment_fc'] = 0
    LDB_prev1['acc_others_charges_payment_myr'] = 0
    ...
    sql_query2 = """INSERT INTO [log_apps_error] (
                ...
                """
    cursor.execute(sql_query2,(str(e)+" ["+str(documentName)+"]","Data Mirror",uploadedByEmail))
    conn.commit()
    sql_error = """UPDATE [jobPython]
    SET [jobCompleted] = NULL, [jobStatus]= 'PY003', [jobErrDetail]= 'Upload Job Clear Data Mirror'
    WHERE [jobName] = 'Job Clear Data Mirror';
                """
    cursor.execute(sql_error)
    conn.commit()
    print(f"Upload Job Clear Data Mirror Error: {e}")
    sys.exit(f"Upload Job Clear Data Mirror Error: {str(e)}")
...
try:
    #table        
    columns = ['aftd_id','result_file_name','processed_status_id','status_id']
    data = [(documentId,"Result_Data_Mirror_"+str(convert_time)[:19]+".xlsx",'PY005','PY002')]
    ...
    create_table_query = "CREATE TABLE A_DATA_MIRROR (" + ', '.join(column_types) + ")"
    # Execute the query
    cursor.execute(create_table_query)

    for row in LDB_prev1.iterrows():
        sql = "INSERT INTO A_DATA_MIRROR({}) VALUES ({})".format(','.join(LDB_prev1.columns), ','.join(['?']*len(LDB_prev1.columns)))
    ...
    cursor.execute("drop table A_DATA_MIRROR")
    conn.commit() 
    ...
    sql_query5 = """INSERT INTO [log_apps_error] (
                ...
                """
    cursor.execute(sql_query5,(str(e)+" ["+str(documentName)+"]","Update Database Job Clear Data Mirror",uploadedByEmail))
    conn.commit()
    sql_error = """UPDATE [jobPython]
    SET [jobCompleted] = NULL, [jobStatus]= 'PY003', [jobErrDetail]= 'Update Job Clear Database Data Mirror'
    WHERE [jobName] = 'Job Clear Data Mirror';
                """
    cursor.execute(sql_error)
    conn.commit()
    print(f"Update Database Job Clear Data Mirror Error: {e}")
    sys.exit(f"Update Database Job Clear Data Mirror Error: {str(e)}")
