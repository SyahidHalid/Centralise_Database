...
try:
    ...
    SET [jobStartDate] = getdate(), [jobStatus]= 'PY001', [PythonFileName]='Job_Clear_Debtor_Listing.py', [jobCompleted] = NULL
    WHERE [jobName] = 'Job Clear Debtor Listing';
                """
...
try:
    ...
    LDB_prev1['acc_accrued_interest_month_fc'] = 0
    LDB_prev1['acc_accrued_interest_month_myr'] = 0
    LDB_prev1['acc_modification_loss'] = 0
    LDB_prev1['acc_modification_loss_myr'] = 0
    LDB_prev1['acc_accurate_interest'] = 0
    LDB_prev1['acc_accrued_interest_myr'] = 0
    LDB_prev1['acc_suspended_interest'] = 0
    LDB_prev1['acc_interest_suspense_myr'] = 0
    LDB_prev1['acc_other_charges'] = 0
    LDB_prev1['acc_other_charges_myr'] = 0
    LDB_prev1['acc_penalty'] = 0
    LDB_prev1['acc_penalty_myr'] = 0
    LDB_prev1['acc_penalty_compensation_fc'] = 0
    LDB_prev1['acc_penalty_compensation_myr'] = 0
    ...
    SET [jobCompleted] = NULL, [jobStatus]= 'PY003', [jobErrDetail]= 'Upload Job Clear Debtor Listing'
    WHERE [jobName] = 'Job Clear Debtor Listing';
                """
    cursor.execute(sql_error)
    conn.commit()
    print(f"Upload Job Clear Debtor Listing Error: {e}")
    sys.exit(f"Upload Job Clear Debtor Listing Error: {str(e)}")
...
try:
    ...
    create_table_query = "CREATE TABLE A_DEBTOR_LISTING (" + ', '.join(column_types) + ")"
    # Execute the query
    cursor.execute(create_table_query)

    for row in LDB_prev1.iterrows():
        sql = "INSERT INTO A_DEBTOR_LISTING({}) VALUES ({})".format(','.join(LDB_prev1.columns), ','.join(['?']*len(LDB_prev1.columns)))
    ...
    cursor.execute("""MERGE INTO col_facilities_application_master AS target USING A_DEBTOR_LISTING AS source
    ...
    cursor.execute("drop table A_DEBTOR_LISTING")
    conn.commit() 
    ...
    sql_query5 = """INSERT INTO [log_apps_error] (
                ...
                """
    cursor.execute(sql_query5,(str(e)+" ["+str(documentName)+"]","Update Database Job Clear Debtor Listing",uploadedByEmail))
    conn.commit()
    sql_error = """UPDATE [jobPython]
    SET [jobCompleted] = NULL, [jobStatus]= 'PY003', [jobErrDetail]= 'Update Job Clear Database Debtor Listing'
    WHERE [jobName] = 'Job Clear Debtor Listing';
                """
    cursor.execute(sql_error)
    conn.commit()
    print(f"Update Database Job Clear Debtor Listing Error: {e}")
    sys.exit(f"Update Database Job Clear Debtor Listing Error: {str(e)}")
