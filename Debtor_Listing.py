...
try:
    ...
    create_table_query_result = "CREATE TABLE Exception_Debtor_Listing (" + ', '.join(column_types1) + ")"
    cursor.execute(create_table_query_result)

    for row in exception_report1.iterrows():
        sql_result = "INSERT INTO Exception_Debtor_Listing({}) VALUES ({})".format(','.join(exception_report1.columns), ','.join(['?']*len(exception_report1.columns)))
        cursor.execute(sql_result, tuple(row[1]))
    conn.commit()
